...
int main(void) {
    ...
    uint16_t version_field = be16_to_cpu(raw.version_be);
    unsigned ver_major = (unsigned)((version_field >> 4) & 0x0Fu);
    unsigned ver_minor = (unsigned)(version_field & 0x0Fu);
    ...
    view.length = length;
    view.reserved = (unsigned)((raw.flags_raw >> 5) & 0x07u);
    view.flags = flags;
